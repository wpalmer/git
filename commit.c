					  ...
					  const char *msg, size_t len)
{
	...
	while (name_end > name_start && isspace(*(name_end-1)))
		name_end--;
	/* 1000 = the size of the buffer in fmt_ident which checks for
	   "Impossibly long personal identifier"s */
	if (name_end-name_start+1 >= 1000)
		goto fail;
	mail_start = msg+end+1;
	mail_end = mail_start;
	while (mail_end < msg_end && *mail_end != '>')
		mail_end++;
	if (mail_end-mail_start+1 >= 1000)
		goto fail;
	if (mail_end == msg_end)
		goto fail;
	end = mail_end-msg;
	...
	parsed->name_len = name_end-name_start+1;
	parsed->email = mail_start;
	parsed->email_len = mail_end-mail_start+1;
